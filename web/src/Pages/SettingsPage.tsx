...
export default function SettingsPage() {
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    if (!newUser.trim() || !newPass.trim()) {
      setFeedback({
        msg: "Veuillez renseigner un nom et un mot de passe.",
        type: "error",
      });
    ...
    if (usersList.some((u) => u.username === newUser.trim())) {
      setFeedback({ msg: "Ce nom d'utilisateur existe déjà.", type: "error" });
      setTimeout(() => setFeedback({ msg: "", type: "" }), 3000);
      return;
    }
    ...
    setNewRole("USER");
    setFeedback({
      msg: `Utilisateur ${userToAdd.username} créé !`,
      type: "success",
    });
    setTimeout(() => setFeedback({ msg: "", type: "" }), 3000);
  };
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto text-white">
      <header className="mb-6 md:mb-8 border-b border-slate-800 pb-4">
        ...
        <p className="text-slate-400 text-sm md:text-base">
          Gestion des accès et de la configuration
        </p>
      </header>
      ...
      <div className="grid gap-6 md:gap-8 grid-cols-1 md:grid-cols-2">
        ...
        <div className="bg-[#0f172a] p-5 md:p-6 rounded-xl border border-slate-800 h-fit">
          ...
          <form onSubmit={handleCreateUser} className="space-y-4">
            ...
            {feedback.msg && (
              <div
                className={`text-sm py-2 px-3 rounded border ${feedback.type === "success" ? "bg-emerald-900/20 border-emerald-900/30 text-emerald-400" : "bg-red-900/20 border-red-900/30 text-red-400"}`}
              >
                {feedback.msg}
              </div>
            )}
      ...
      <div className="bg-[#0f172a] p-5 md:p-6 rounded-xl border border-slate-800">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            ...
            {usersList.map((user, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 bg-[#1e293b] rounded-lg border border-slate-700 group hover:border-slate-600 transition-colors"
              >
        ...
        <div className="mt-6 pt-6 border-t border-slate-800">
          <p className="text-slate-400 text-sm flex items-center gap-2">
            <LogOut size={16} />
            Pour créer un nouveau compte, utilisez la page d'inscription
