...
export default function InfraNode({ data, selected }: NodeProps) {
  ...
  const statusClassName =
    data.status === "offline"
      ...
      : data.status === "missing"
        ? { ping: "bg-yellow-400", dot: "bg-yellow-500" }
        : data.status === "warning"
    ...
    <div className="flex flex-col items-center gap-2 group">
      ...
      <div className="relative">
        ...
        <div className="absolute top-0 right-0 z-20">
          <span className="relative flex h-3 w-3">
            <span
              className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${statusClassName.ping}`}
            ></span>
            <span
              className={`relative inline-flex rounded-full h-3 w-3 ${statusClassName.dot}`}
            ></span>
          </span>
        </div>
      </div>
      ...
      <div className="text-center z-10">
        <div className="text-slate-200 text-[11px] font-bold tracking-wide flex items-center justify-center gap-1">
          ...
          {data.role === "SERVER" && (
            <Activity size={10} className="text-emerald-500 animate-bounce" />
          )}
        </div>
