...
interface AppStoreType {
  ...
  launch_scan: (target?: string) => Promise<'success' | 'missing-target' | 'error'>;
}
...
const getInitialScanDataStatus = (): ScanDataStatus => {
  ...
  if (saved === 'none' || saved === 'valid' || saved === 'outdated') return saved;
...
const getInitialScanTarget = (): string | null => {
  return localStorage.getItem('last_scan_target') ?? localStorage.getItem('currentScanTarget');
};

export function AppStoreProvider({ children }: { children: ReactNode }) {
  const [auth_status, setAuthStatus] = useState<boolean>(getInitialAuthStatus);
  const [subscription_tier, setSubscriptionTier] = useState<SubscriptionTier>(getInitialSubscriptionTier);
  const [scan_data_status, setScanDataStatus] = useState<ScanDataStatus>(getInitialScanDataStatus);
  const [last_scan_target, setLastScanTargetState] = useState<string | null>(getInitialScanTarget);
  const [is_scanning, setIsScanning] = useState(false);
  const [scan_target_prompt_requested, setScanTargetPromptRequested] = useState(false);
  ...
  const launch_scan = useCallback(async (target?: string) => {
